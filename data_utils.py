...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        assert abs(c.size(-1) - spec.size(-1)) < 4, (c.size(-1), spec.size(-1), filename)
        assert abs(lmin - spec.size(-1)) < 4, (c.size(-1), spec.size(-1))
        assert abs(lmin - c.size(-1)) < 4, (c.size(-1), spec.size(-1))
...
class EvalDataLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        lmin = min(c.size(-1), spec.size(-1))
        assert abs(c.size(-1) - spec.size(-1)) < 4, (c.size(-1), spec.size(-1))
        spec, c = spec[:, :lmin], c[:, :lmin]
        audio_norm = audio_norm[:, :lmin * self.hop_length]
